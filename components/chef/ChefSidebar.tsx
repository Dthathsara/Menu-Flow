...
import type { ChefNavItem, ChefNavKey } from "./types";
import { getChefNavIcon } from "./chef-utils";
...
export function ChefSidebar({
  ...
  navItems: ChefNavItem[];
  overlay?: boolean;
  open?: boolean;
  onSelect: (key: ChefNavKey) => void;
  onClose?: () => void;
}) {
  ...
  function handleNavigate(item: ChefNavItem) {
    onSelect(item.key);
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <nav className="mt-4 flex-1 space-y-1.5">
            {navItems.map((item) => {
              const Icon = getChefNavIcon(item.icon);
