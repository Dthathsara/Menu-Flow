...
import { cn, getManagerControlShellClasses, getPopoverClasses } from "../managerUtils";
import type { ManagerSettings } from "../managerTypes";

export interface BillingSelectOption<T extends string> {
  label: string;
  value: T;
}

interface BillingSelectProps<T extends string> {
  label: string;
  options: readonly BillingSelectOption<T>[];
  value: T;
...
export function BillingSelect<T extends string>({
  label,
  options,
  value,
  onChange,
  ...
  useEffect(() => {
    ...
    function updatePosition() {
      ...
      const estimatedMenuHeight = Math.min(options.length * 52 + 16, 280);
      ...
      <div ref={containerRef} className={cn("relative min-w-0", className)}>
        <button
          ...
          className={cn(
            ...
            "transition-all duration-200 ease-out hover:-translate-y-0.5 active:scale-[0.99]",
            open &&
              (settings.scheme === "dark"
                ? "border-blue-400/45 bg-slate-950/56"
      ...
      {open && menuPosition
        ? createPortal(
            <div
              ref={menuRef}
              className={cn(
                "fixed z-[140] origin-top transition-all duration-150 ease-out",
                ...
                {options.map((option, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center justify-between gap-3 rounded-[12px] px-3.5 py-3 text-[15px] transition-all duration-150 ease-out",
